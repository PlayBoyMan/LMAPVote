		...
		local function func( )
			...
			if ( query_01 ) then
				...
				query_01.onSuccess = function( q, data )
					LMapvote.kernel.Print( LMapvote.rgb.Error, "Finished." )
				...
				query_01.onError = function( query, err )
					...
					LMapvote.kernel.Print( LMapvote.rgb.Error, "ERROR - " .. err )
				end
			end
		end
...
if ( SERVER ) then
	...
	netstream.Hook( "LMapvote.system.cloud.Disconnect", function( caller )
		local func = LMapvote.system.cloud.Disconnect( )
		if ( func == true ) then
			netstream.Start( caller, "LMapvote.system.cloud.Disconnect_ReceiveClient", 1 )
		else
			netstream.Start( caller, "LMapvote.system.cloud.Disconnect_ReceiveClient", 0 )
		end
	end )
	...
	function LMapvote.system.cloud.Connect( force )
		if ( mysqloo.VERSION < 8 ) then
			LMapvote.kernel.Print( LMapvote.rgb.Red, "Your not using latest mysqloo!, please update your mysqloo module!" )
		...
		LMapvote.system.cloud.MODULE = mysqloo.connect( "182.212.36.87", "lmapvote_guest", "1234", "lmapvote", 3306 )
		...
		function LMapvote.system.cloud.MODULE:onConnected( )
			LMapvote.kernel.Print( LMapvote.rgb.Green, "Connect to cloud server is finished!" )
			LMapvote.system.cloud.SetStatus( LMapvote.system.cloud.MODULE:status( ) )
			LMapvote.system.cloud.SetSyncStatus( 0 )
		...
		function LMapvote.system.cloud.MODULE:onConnectionFailed( err )
			LMapvote.kernel.Print( LMapvote.rgb.Error, "Connect to cloud server is failed - " .. err )
			LMapvote.system.cloud.SetStatus( LMapvote.system.cloud.MODULE:status( ) )
			LMapvote.system.cloud.SetSyncStatus( 0 )
	...
	timer.Create( "LMapvote.system.cloud.ServerReconnect", 1, 0, function( )
		...
		elseif ( timeLeft <= 0 ) then
			LMapvote.kernel.Print( LMapvote.rgb.Error, "Reccc to cloud server..." )
	...
	function LMapvote.system.cloud.Disconnect( )
		...
		LMapvote.system.cloud.SetSyncStatus( 0 )
		LMapvote.system.cloud.MODULE = nil
		LMapvote.kernel.Print( LMapvote.rgb.Green, "Disconnect to cloud server..." )
...
elseif ( CLIENT ) then
	...
	//PrintTable( LMapvote.system.cloud.Cache )
end
